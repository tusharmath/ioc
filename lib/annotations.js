...
function getAnnotationName(annotationName) {
    let keyName = annotationName;
    return keyName;
}
class AnnotatedClass {
    ...
    _create(annotations) {
        let results, _this = this;
        results = [];
        //_.each(annotations, (k,v) => console.log(k, v));
        for (let annotationName in annotations) {
            let annotationValue = annotations[annotationName];
            let annotationFunction = function () {
                var args = _.toArray(arguments);
                return _this._applyAnnotation(ANNOTATIONS[annotationName], args);
            };
            var keyName = getAnnotationName(annotationName);
            results.push(this[keyName] = annotationFunction);
        }
        return results;
    }
}
